...
const Button: React.FC<ButtonProps> = ({children, disabled, onClick, btnType, style}): JSX.Element => { 
    ...
    return (
        <ButtonWrapper disabled={disabled}>
            <button style={style} ref={buttonNode} type={btnType} onClick={onClick}>{children}</button>
            {state.modal.isModalOpen?<ModalWarning associatedButton={buttonNode?.current}>{state.modal.modalMessage}</ModalWarning>:<></>}
        </ButtonWrapper>
    )
}
